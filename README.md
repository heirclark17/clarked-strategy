...
- **`backend/`** — FastAPI service: contact form/email, accounts + auth, Stripe payments, and a OpenAI-powered AI endpoint (deploys to **Railway**).
...
4. Add environment **Variables** (see `backend/.env.example`). At minimum set a strong `JWT_SECRET`:
   ...
   ```
   Add `RESEND_API_KEY`, `STRIPE_SECRET_KEY`, `OPENAI_API_KEY` when you want those features on.
...
| POST   | `/ai`               | JWT  | OpenAI-powered generation (503 until configured) |
...
| AI        | `OPENAI_API_KEY` | https://platform.openai.com |
